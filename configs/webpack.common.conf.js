...
const getEntryFile = dir => {
  ...
  const enrtys = glob.sync(
    `${dir}/${config.entryFilter}`,
    config.entryFilterOptions
  )
  enrtys.forEach(entry => {
    ...
    fs.outputFileSync(
      templatePathForWeb,
      getWebEntryFileContent(templatePathForWeb, entry)
    )
    fs.outputFileSync(
      templatePathForNative,
      getNativeEntryFileContent(templatePathForNative, entry)
    )
