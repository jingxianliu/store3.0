...
export default {
  ...
  created () {
    ...
    this.versionCode = this.getQueryString('versionCode')

    if (this.getQueryString('isEPG')) {
      this.isEPG = true
    }
    console.log(`daixin:-----packageName${packageName},versionCode${this.versionCode}`)
  ...
  methods: {
    ...
    autoFocus (code) {
      ...
      console.log(`app详情页面-----${this.focusCode}----------${code}`)
      if (code === 4) {
       
       console.log(`app详情页面点击返回-----${ this.show}--`)

        if (this.show) {
          // 视频关闭全屏
          this.$refs.simpleVideo.stop()
          this.show = false          
        } else {
          // 返回
    ...
    startApp () {
      ...
      if (isLocal && !(this.versionCode - isLocal > 0)) {
        // 1. 判断应用是否存在
